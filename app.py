...
MAINTENANCE_FILE = os.path.join(DATABASE_DIR, 'maintenance.json')
LOOT_SADIQ_FILE = os.path.join(DATABASE_DIR, 'loot_sadiq.json')
...
<body>
    <div class="maintenance-container">
        ...
        <div class="status">
            <i class="fas fa-spinner fa-spin"></i> جاري الصيانة
        </div>
        <div class="message">
            {{ message }}
        </div>
        <p style="color: #666666; font-size: 14px; margin-top: 30px;">
            سيعود الموقع قريباً. شكراً لصبرك!
        </p>
    </div>
</body>
</html>
...
def get_maintenance():
    return _load_json(MAINTENANCE_FILE, {'enabled': False, 'message': 'الموقع قيد الصيانة الآن'})

def save_maintenance(data):
    _save_json(MAINTENANCE_FILE, data)

def get_loot_sadiq():
    return _load_json(LOOT_SADIQ_FILE, [])

def save_loot_sadiq(data):
    _save_json(LOOT_SADIQ_FILE, data)
...
def index():
    ...
    if maintenance.get('enabled'):
        return render_template_string(TPL_MAINTENANCE, message=maintenance.get('message', 'الموقع قيد الصيانة الآن'))
...
def admin():
    ...
    return render_template_string(TPL_ADMIN,
        ...
        maintenance=get_maintenance(), loot_count=len(get_loot_sadiq()))
...
def maintenance_settings():
    ...
    if request.method == 'POST':
        ...
        maintenance = {
            ...
            'message': data.get('message', 'الموقع قيد الصيانة الآن'),
            'updated_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        }
        save_maintenance(maintenance)
        return jsonify({'success': True, 'message': 'تم تحديث إعدادات الصيانة'})
    return jsonify({'success': True, 'maintenance': get_maintenance()})

@app.route('/admin/loot_sadiq/upload', methods=['POST'])
def upload_loot_sadiq():
    if 'user_id' not in session or not session.get('is_admin'):
        return jsonify({'success': False, 'error': 'Unauthorized'})
    ...
    try:
        loot_data = {
            'id': len(get_loot_sadiq()) + 1,
            'filename': f.filename,
            'uploaded_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            'visible': False
        }
        loot_folder = os.path.join(BOTS_STORAGE, 'loot_sadiq')
        os.makedirs(loot_folder, exist_ok=True)
        file_path = os.path.join(loot_folder, f.filename)
        f.save(file_path)
        loots = get_loot_sadiq()
        loots.append(loot_data)
        save_loot_sadiq(loots)
        return jsonify({'success': True, 'message': 'تم رفع اللوت بنجاح', 'loot': loot_data})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})

@app.route('/admin/loot_sadiq', methods=['GET'])
def get_loot_sadiq_list():
    if 'user_id' not in session or not session.get('is_admin'):
        return jsonify({'success': False, 'error': 'Unauthorized'})
    return jsonify({'success': True, 'loots': get_loot_sadiq()})

@app.route('/admin/loot_sadiq/<int:loot_id>/delete', methods=['POST'])
def delete_loot_sadiq(loot_id):
    if 'user_id' not in session or not session.get('is_admin'):
        return jsonify({'success': False, 'error': 'Unauthorized'})
    loots = get_loot_sadiq()
    loot = next((l for l in loots if l['id'] == loot_id), None)
    if not loot:
        return jsonify({'success': False, 'error': 'اللوت غير موجود'})
    try:
        file_path = os.path.join(BOTS_STORAGE, 'loot_sadiq', loot['filename'])
        if os.path.exists(file_path):
            os.remove(file_path)
        loots = [l for l in loots if l['id'] != loot_id]
        save_loot_sadiq(loots)
        return jsonify({'success': True, 'message': 'تم حذف اللوت بنجاح'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})

@app.route('/admin/loot_sadiq/<int:loot_id>/toggle', methods=['POST'])
def toggle_loot_sadiq_visibility(loot_id):
    if 'user_id' not in session or not session.get('is_admin'):
        return jsonify({'success': False, 'error': 'Unauthorized'})
    loots = get_loot_sadiq()
    loot = next((l for l in loots if l['id'] == loot_id), None)
    if not loot:
        return jsonify({'success': False, 'error': 'اللوت غير موجود'})
    loot['visible'] = not loot.get('visible', False)
    save_loot_sadiq(loots)
    return jsonify({'success': True, 'message': 'تم تحديث الرؤية', 'visible': loot['visible']})

@app.route('/admin/loot_sadiq/<int:loot_id>/download')
def download_loot_sadiq(loot_id):
    if 'user_id' not in session or not session.get('is_admin'):
        return abort(403)
    loots = get_loot_sadiq()
    loot = next((l for l in loots if l['id'] == loot_id), None)
    ...
    if not os.path.exists(file_path):
        return abort(404)
    return send_file(file_path, as_attachment=True, download_name=loot['filename'])
